...
import com.badbones69.crazyenvoys.multisupport.SkullCreator;
...
public class ItemBuilder implements Cloneable {
    ...
    private static final boolean useNewMaterial = ServerProtocol.isNewer(ServerProtocol.v1_12_R1);
    ...
    public ItemBuilder setMaterial(String materialString) {
        ...
        if (material != null) { // Sets the material.
            this.material = material;
            // 1.9-1.12.2
            if (ServerProtocol.isNewer(ServerProtocol.v1_8_R3) && ServerProtocol.isOlder(ServerProtocol.v1_13_R2) && material == Material.matchMaterial("MONSTER_EGG")) {
                try {
    ...
    public ItemStack build() {
        ...
        if (item.getType() != Material.AIR) {
            ...
            if (ServerProtocol.isSame(ServerProtocol.v1_8_R3) && isHead && !isHash && player != null && !player.equals("")) {
            ...
            if (isHead && !isHash && player != null && !player.equals("") && ServerProtocol.isNewer(ServerProtocol.v1_8_R3)) {
            ...
            if (unbreakable && ServerProtocol.isOlder(ServerProtocol.v1_11_R1)) {
    ...
    private static Enchantment getEnchantment(String enchantmentName) {
        ...
        for (Enchantment enchantment : Enchantment.values()) {
            try {
                ...
                if (ServerProtocol.isNewer(ServerProtocol.v1_12_R1) && stripEnchantmentName(enchantment.getKey().getKey()).equalsIgnoreCase(enchantmentName)) {
                    return enchantment;
                }
